...
while running:
    ...
    try:
        ...
        new_player = Player(new_socket, addr, random.randint(0, WIDTH_ROOM), random.randint(0, HEIGHT_ROOM),
                            START_PLAYER_R, str(random.randint(0, 4)))
    ...
    for i in range(len(players)):
        ...
        # print(top)
        responses[i] = '<' + (
            ','.join([r_ + ' ' + x_ + ' ' + y_ + ' ' + L_] + visible_balls[i] + [str(len(players))] + [top])) + '>'
        # print(responses[i])
